...
for pair in all_pairs:
    ...
    start_row = min(pair[0][0], pair[1][0])
    end_row = max(pair[0][0], pair[1][0])

    empty_rows = 0

    for c in inf_rows:
        if c > start_row and c < end_row:
            empty_rows += 1


    start_column = min(pair[0][1], pair[1][1])
    end_column = max(pair[0][1], pair[1][1])

    empty_cols = 0

    for r in inf_cols:
        if r > start_column and r < end_column:
            empty_cols += 1

    d = np.abs(pair[0][0] - pair[1][0]) + empty_rows * (1000000 - 1) + \
        np.abs(pair[0][1] - pair[1][1]) + empty_cols * (1000000 - 1)

    print(start_column, end_column, pair, d)
